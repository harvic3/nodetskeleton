...
export default class TextFeelingRepo implements IFeelingRepository {
  async AnaliceText(text: string): Promise<ITextFeeling> {
      ...
      content.text = text;
      try {
        const tResponse = await HttpClient.SendAsync<TextFeelinRepoDto>(textFeelingApi, HttpClient.Method.POST, content, headers);
        if (!tResponse.success){
          throw HttpError(tResponse.statusCode || 500, tResponse.message);        
        }
        const result = new TextFeeling(text);
        result.sentiment = new Sentiment(tResponse.response.result.polarity, tResponse.response.result.type);
